...
//added by ke
import org.apache.hadoop.hdfs.server.namenode.raidClient;
/**
...
public class BlockPlacementPolicyRaid extends BlockPlacementPolicyDefault {
  ...
  //added by ke 2016/2/29 starts 
  public static int flag=0;
  public static HashSet<String> favoredRacks = new HashSet<String>();
  public static HashSet<String> raidRacks =new HashSet<String>();
  ...
  public void initialize(Configuration conf,  FSClusterStats stats,
    ...
    this.raid=true;
    this.raidclient=new raidClient();
  }
//added by ke
  /**
  ...
  private void initParityConfigs() {
    String acceptedCodecId = "xor";
    for (Codec c : Codec.getCodecs()) {
      if (acceptedCodecId.equals(c.id)) {
        FSNamesystem.LOG.info("Parity info."
          + " Id: " + c.id
    ...
    System.out.println("stripelength="+stripeLen);
  }
//ends
  ...
  public DatanodeDescriptor[] chooseTarget(String srcPath, int numOfReplicas,
    ...
    try {
      ...
      //added by ke 2016/3/9    
      //added by ke 2016/3/2
      System.out.println("**********************");
      System.out.println("choosetarget+raid");
      flag++;
      //ends
      ...
      for (Node node:chosenNodes) {
       ...
       System.out.println("raid="+raid);
      if (info.type == FileType.NOT_RAID) {
      //added by ke ,2016/3/9
        System.out.println("fileType=NOT_RAID");       
        if(flag==1)
        {       
          System.out.println("flag==1,chooserandom"+flag);
	  //raid=false;
   	  chooseRandom(numOfReplicas, Path.SEPARATOR, excludedNodes, blocksize,1, results);
          for (DatanodeDescriptor node : results) {
   		  String rack = node.getNetworkLocation();
  	 	  favoredRacks.add(rack);
                  raidRacks.add(rack);
      		}
      }
      else 
      {
          System.out.println("flag=="+flag+",chooserandom");
          Iterator<String> it = favoredRacks.iterator();
          for(int i=0;i<numOfReplicas;i++)
	  {
           excludedNodes.clear();
	   chooseRandom(1,it.next(),excludedNodes, blocksize, 
            1, results);
          }
      ...
      if(flag==stripeLen)
	{
                System.out.println("flag="+stripeLen);
                raid=true;
                raidclient.sendraidflag();
                System.out.println("raid="+raid);
		favoredRacks.clear();
                flag=0;              
	}
      ...
      //added by ke 2016/3/29
      //parity block placement
         else {   
          System.out.println("info.type="+info.type);  
   	  Iterator<String> it = raidRacks.iterator();
          for(int i=0;i<numOfReplicas;i++)
	  {
	      chooseRandom(1,it.next(), excludedNodes, blocksize,1, results);
          }
          //raidRacks.clear();
         return results.toArray(new DatanodeDescriptor[results.size()]);
      } 
      ...
      System.out.println("error happened when choosing datanode to write:"+ StringUtils.stringifyException(e));
  ...
  public DatanodeDescriptor chooseReplicaToDelete(FSInodeInfo inode,
    ...
    try {
      System.out.println("choosereplicatodelete in raid");
      String path = getFullPathName(inode);
      FileInfo info = getFileInfo(inode, path);
      if (info.type == FileType.NOT_RAID) {
        System.out.println("choosereplicatodelete in raid when filetype==not_raid");
      ...
      if (companionBlocks == null || companionBlocks.size() == 0) {
        System.out.println("choosereplicatodelete in raid when there is no valid raided or parity file");
      ...
      all.addAll(first);
      all.addAll(second);
      System.out.println("use private method in raid to delete");
      chosenNode = chooseReplicaToDelete(companionBlocks, all);
      if (chosenNode != null) {
        return chosenNode;
      }
      System.out.println("fina return in this choosereplicatodelete method");
      return super.chooseReplicaToDelete(
          inode, block, replicationFactor, first, second);
    } catch (Exception e) {
      LOG.debug("Failed to choose the correct replica to delete", e);
      System.out.println("Failed to choose the correct replica to delete,return default");
      return super.chooseReplicaToDelete(
          inode, block, replicationFactor, first, second);
    }
  }
